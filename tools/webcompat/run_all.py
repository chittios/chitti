...
def run_realworld_fixtures(bin_path: Path) -> dict:
    """Parse every real-world script fixture through the `just` tier and report
    parse pass/fail. These are actual scripts shipped by production sites
    (jQuery/lodash minified, Vercel chunks, and every css3test.com subresource) —
    a regression net for grammar coverage the synthetic test262 corpus misses
    (e.g. multi-line method chains, which broke css3test's `supports.js`).

    ESM modules (top-level `export`/`import`) are parsed by the browser tier's
    `strip_module_syntax` before reaching this strict parser, so the raw runner
    would reject them structurally — they're reported separately as `esm`, not
    failures."""
    if not FIXTURES.exists():
        return {"error": "fixtures dir missing", "pass": 0, "fail": 0, "esm": 0}
    files = sorted(FIXTURES.rglob("*.js"))
    passed, failed, esm = 0, 0, 0
    fails: list[str] = []
    for f in files:
        try:
            ...
            continue
        # ESM entry files (`export default …`, `import … from`) are the browser
        # tier's job (strip_module_syntax), not the strict grammar's. A module
        # may lead with a data preamble (`const … = […]`) before its top-level
        # `export`, so scan for a line-anchored export/import rather than only
        # the first token.
        if re.search(r"(?m)^\s*(export|import)\s", text):
            esm += 1
            continue
        r = run([str(bin_path), "--raw", str(f)], timeout=60)
        out = (r.stdout or "") + (r.stderr or "")
        # A parse failure surfaces as a SKIP with a ParserValidation error; a
        # clean parse either PASSes or FAILs at runtime (missing browser globals),
        # both of which mean the grammar accepted the script.
        if "ParserValidation" in out or "panicked" in out:
            failed += 1
            fails.append(f"{f.relative_to(FIXTURES)}")
        else:
            passed += 1
    return {"pass": passed, "fail": failed, "esm": esm, "fails": fails,
            "total": len(files)}
...
def main():
    ...
    # Real-world script fixtures — grammar regression net (parse pass/fail on
    # actual production scripts, incl. every css3test.com subresource).
    if just_bin:
        print("Parsing real-world fixtures (jQuery/lodash/vercel/css3test)…")
        rw = run_realworld_fixtures(just_bin)
        lines.append("## Real-world script fixtures — `just` parse regression")
        lines.append("")
        lines.append(
            "Every script under `tools/webcompat/fixtures/` (production jQuery/"
            "lodash, Vercel chunks, and all css3test.com subresources) parsed by "
            "the `just` tier via `--raw`. A grammar regression net for constructs "
            "test262 misses — e.g. multi-line method chains (`foo\\n.bar()`), "
            "which broke css3test's `supports.js`."
        )
        lines.append("")
        if rw.get("error"):
            ...
            lines.append(
                f"- Scripts parsed: **{rw['pass']}** ok / **{rw['fail']}** fail "
                f"(+ **{rw['esm']}** ESM entry files handled by the browser tier's "
                f"`strip_module_syntax`, not the strict grammar)"
            )
            if rw.get("fails"):
                lines.append("- **Parse failures:**")
                for s in rw["fails"][:40]:
                    lines.append(f"  - `{s}`")
        lines.append("")
