...
const DEFAULT_SHORTCUTS: &[(&str, &str, &str)] = &[
    ...
    // Each shortcut lists the chord first and the function key second. An Apple
    // keyboard has no Print Screen key and needs Fn for a bare F-key, so the
    // chord is the reachable form there; the F-key is what a serial terminal can
    // send. Ctrl is the twin for when a macOS host steals the Cmd one.
    ("Cmd+/ | Ctrl+/ | F1", "help", "open the Commands browser (same as /help)"),
    (
