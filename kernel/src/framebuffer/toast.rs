...
pub fn toast_show(sev: crate::notify::Severity, head: &str, lines: &[alloc::string::String]) {
    SCREEN.with(|slot| {
        ...
        sc.paint_surface(x, y, w, h, sc.theme.composer_bg);
        let accent = severity_color(sc, sev);
        sc.rounded_outline(x, y, w, h, (4 * sc.scale).max(4), accent);
        // A severity stripe down the left edge: the one piece of colour that
        // says *what kind* of notification this is before any text is read.
        sc.fill_rect(x + 2, y + 3, (2 * sc.scale).max(2), h.saturating_sub(6), accent);
        ...
        sc.draw_str(tx, ty, &heading, accent, sc.theme.composer_bg);
        ty += ch + ch / 3;
        for l in lines {
            sc.draw_str(tx, ty, l, sc.theme.chat_fg, sc.theme.composer_bg);
            ty += ch;
        }
        sc.cursor_overlay();
...
fn severity_color(sc: &Screen, sev: crate::notify::Severity) -> Rgb {
    use crate::notify::Severity;
    match sev {
        // Warn/Error/Action get the accent, which is the theme's "look here"
        // colour; Info and Success recede. Deliberately not hard-coded amber/red:
        // `/theme` must be able to recolour every surface, and a banner with a
        // baked palette would be the one thing that ignored it.
        Severity::Error | Severity::Warn | Severity::Action => sc.theme.accent,
        Severity::Success => sc.theme.title_dim,
        Severity::Info => sc.theme.sep_dim,
    }
}
