...
/// A **second** page, for the one HGCM call that is deliberately left pending.
///
/// `MSG_OLD_GET_WAIT` does not complete until the host has something to say, so
/// its request buffer stays owned by the host indefinitely. Sharing it with the
/// synchronous calls that run in the meantime would let one overwrite a request
/// the host is still holding a pointer to.
static mut MSG_PAGE: ReqPage = ReqPage([0; 4096]);
...
pub fn bring_up() -> Result<String, &'static str> {
    ...
    // The transport is up, so the services on it can be reached. The shared
    // clipboard is the one worth having; a failure here is reported, not fatal
    // to bring-up, because the transport itself is still usable.
    if let Err(e) = clipboard::connect() {
        crate::ktrace::log_fmt(format_args!("vbox: shared clipboard unavailable: {e}"));
    }
    Ok(v)
}
    ...
    DEV.with(|d| match d.as_ref() {
        ...
        Some(g) => {
            ...
            match &g.host_version {
                Some(v) => alloc::format!(
                    "VMMDev at {:02x}:{:02x}.{} via {w}; host {v}. {}",
                    g.bus,
                    g.dev,
                    g.func,
                    clipboard::status().unwrap_or_else(|| alloc::string::String::from(
                        "shared clipboard not connected (shared folders are not implemented)"
                    ))
                ),
                None => alloc::format!(
...
/// How long to wait for a synchronous HGCM call to complete.
///
/// Bounded and interruptible, per the standing rule. An HGCM call the host
/// never finishes would otherwise wedge the shell with no way out.
const HGCM_SPIN_LIMIT: u64 = 200_000_000;

/// The two low pages, as `(phys, virt)` slices.
///
/// SAFETY-note: each is used by exactly one path — `REQ_PAGE` by the
/// synchronous calls (serialised by being called from one cooperative task),
/// `MSG_PAGE` by the single outstanding message wait.
fn req_page() -> (u64, &'static mut [u8]) {
    // SAFETY: a page of `.bss` owned by this driver; no other reference exists.
    unsafe {
        let p = core::ptr::addr_of_mut!(REQ_PAGE) as *mut u8;
        (p as u64, core::slice::from_raw_parts_mut(p, 4096))
    }
}

fn msg_page() -> (u64, &'static mut [u8]) {
    // SAFETY: as above, for the message-wait page.
    unsafe {
        let p = core::ptr::addr_of_mut!(MSG_PAGE) as *mut u8;
        (p as u64, core::slice::from_raw_parts_mut(p, 4096))
    }
}

/// Submit a framed request and wait for the host to finish with it.
///
/// The VMMDev register write is synchronous for ordinary requests but only
/// *starts* an HGCM one, so this then polls `fu32Flags` for `REQ_DONE`. Reading
/// the outputs on the strength of the register write returning would read
/// whatever was in the buffer before.
fn submit_and_wait(phys: u64, buf: &[u8]) -> Result<(), &'static str> {
    if !submit(phys) {
        return Err("the request register is unreachable");
    }
    crate::drivers::virtio::barrier();
    ...
    if hgcm::result(buf) < 0 {
        return Err("the service returned an error");
    }
    Ok(())
}

/// Connect to an HGCM service by name; returns the client id.
pub fn hgcm_connect(service: &str) -> Result<u32, &'static str> {
    if !present() && !probe() {
        return Err("no VirtualBox VMMDev device on this machine");
    }
    let (phys, buf) = req_page();
    if !request_addr_fits(phys) {
        return Err("request buffer is above 4 GiB");
    }
    if !hgcm::write_connect(buf, service) {
        return Err("could not frame the HGCM connect");
    }
    submit_and_wait(phys, buf)?;
    let id = hgcm::connect_client_id(buf);
    if id == 0 {
        return Err("the host connected us but assigned no client id");
    }
    Ok(id)
}

/// Make a synchronous HGCM call. Returns the **last** parameter read back as a
/// `u32`, which is what the service functions here use for a returned size.
pub fn hgcm_call(client: u32, function: u32, parms: &[hgcm::Parm]) -> Result<u32, &'static str> {
    let (phys, buf) = req_page();
    if !hgcm::write_call(buf, client, function, parms) {
        return Err("could not frame the HGCM call");
    }
    submit_and_wait(phys, buf)?;
    Ok(hgcm::parm_u32(buf, parms.len().saturating_sub(1)))
}

/// Post an HGCM call on the message page and **leave it pending**.
///
/// Returns whether the host accepted it. The reply is collected later by
/// [`hgcm_take_message`] — this is the polled stand-in for the blocking thread
/// a real guest agent uses.
pub fn hgcm_post_message_wait(client: u32, function: u32, parms: &[hgcm::Parm]) -> bool {
    let (phys, buf) = msg_page();
    if !request_addr_fits(phys) || !hgcm::write_call(buf, client, function, parms) {
        return false;
    }
    if !submit(phys) {
        return false;
...
///
/// `Some((message_id, formats))` when a message arrived; `None` while it is
/// still pending, which is the common case and must stay cheap — it is one
...
pub fn hgcm_take_message() -> Option<(u32, u32)> {
    ...
    if hgcm::result(buf) < 0 {
        // Consumed and failed: clear the done flag so the caller re-arms
        // instead of reading this reply forever.
        buf[REQ_HDR] = 0;
        return None;
    }
    let msg = hgcm::parm_u32(buf, 0);
    let formats = hgcm::parm_u32(buf, 1);
    buf[REQ_HDR] = 0;
    Some((msg, formats))
}
