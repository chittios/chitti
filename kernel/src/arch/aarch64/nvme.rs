//! **NVMe block driver over PCIe** — the modern real-hardware storage path (the
//! disk most laptops/servers and hypervisors like VirtualBox expose when not
//! using virtio). Found on the ACPI-discovered PCIe bus (`crate::pci`), same as
//! the virtio-pci and xHCI drivers. Polled admin + one I/O queue pair; read/
//! write via PRP lists; presented behind the shared [`BlockDevice`] API.
...
fn alloc_ident(bytes: usize) -> u64 {
    ...
    assert!(!p.is_null(), "nvme: DMA alloc failed");
    p as u64
}
...
/// A polled NVMe controller + its single I/O queue pair.
pub struct Nvme {
    regs: u64,
    ...
    io_phase: u32,
    cid: u16,
    prp_list: u64, // one page for PRP lists on multi-page transfers
    data_buf: u64, // 64 KiB bounce buffer
}
...
impl Nvme {
    pub fn probe_nth(n: usize) -> Option<Nvme> {
        ...
        unsafe { Self::bringup(d, regs) }
    }

    unsafe fn bringup(_d: pci::PciDevice, regs: u64) -> Option<Nvme> {
        unsafe {
            ...
            // Admin queues.
            let asq = alloc_ident(QDEPTH * SQE);
            let acq = alloc_ident(QDEPTH * CQE);
            w32(regs + REG_AQA, (((QDEPTH - 1) as u32) << 16) | (QDEPTH - 1) as u32);
            w64(regs + REG_ASQ, dma(asq));
            w64(regs + REG_ACQ, dma(acq));
            ...
            let mut nv = Nvme {
                ...
                a_cq_head: 0,
                a_phase: 1,
                iosq: alloc_ident(QDEPTH * SQE),
                iocq: alloc_ident(QDEPTH * CQE),
                io_sq_tail: 0,
                io_cq_head: 0,
                io_phase: 1,
                cid: 0,
                prp_list: alloc_ident(4096),
                data_buf: alloc_ident(DATA_MAX),
            };

            // Create the I/O completion + submission queues (qid 1).
            let iocq_pa = dma(nv.iocq);
            // Create IO CQ: opcode 0x05; CDW10 = (size-1)<<16 | qid; CDW11 = PC(1).
            if !nv.admin(0x05, 0, iocq_pa, 0, (((QDEPTH - 1) as u32) << 16) | 1, 1) {
                return None;
            }
            let iosq_pa = dma(nv.iosq);
            // Create IO SQ: opcode 0x01; CDW11 = cqid(1)<<16 | PC(1).
            if !nv.admin(0x01, 0, iosq_pa, 0, (((QDEPTH - 1) as u32) << 16) | 1, (1 << 16) | 1) {
                return None;
            }

            // Identify Namespace 1 (CNS=0) for capacity + LBA size.
            let idbuf = alloc_ident(4096);
            if !nv.admin(0x06, 1, dma(idbuf), 0, 0, 0) {
                return None;
            }
            // NSZE (u64 @ 0) = number of LBAs; FLBAS (@26) selects the LBA format;
            // LBA formats start @128, each u32, LBADS (byte 2) = log2(lba bytes).
            let nsze = read_volatile(idbuf as *const u64);
            let flbas = read_volatile((idbuf + 26) as *const u8) & 0xf;
            let lbaf = read_volatile((idbuf + 128 + flbas as u64 * 4) as *const u32);
    ...
    unsafe fn submit(&mut self, io: bool, opcode: u8, nsid: u32, prp1: u64, prp2: u64, cdw10: u32, cdw11: u32, cdw12: u32, _r: u32) -> bool {
        unsafe {
            ...
            let e = sq + (*tail as u64) * SQE as u64;
            ...
            *tail = (*tail + 1) % QDEPTH as u32;
            ...
            let mut spins = 0u64;
            loop {
                let c = cq + (*head as u64) * CQE as u64;
    ...
    fn rw(&mut self, write: bool, index: u64, ptr: *mut u8, len: usize) -> Result<(), BlockError> {
        ...
        unsafe {
            ...
            let prp1 = dma(self.data_buf);
            let pages = len.div_ceil(4096);
            let prp2 = if pages <= 1 {
                0
            } else if pages == 2 {
                dma(self.data_buf + 4096)
            } else {
                // PRP list: entries for pages 2..N (page 1 is PRP1).
                for i in 0..pages - 1 {
                    w64(self.prp_list + i as u64 * 8, dma(self.data_buf + (i as u64 + 1) * 4096));
                }
                dma(self.prp_list)
            };
