//! **AHCI (SATA) block driver over PCIe** — the storage controller VirtualBox
//! defaults to and most SATA hardware exposes. Found on the ACPI-discovered
//! PCIe bus (`crate::pci`, class 0x01/0x06/0x01), ABAR at BAR5. Polled,
//! single-port, one command slot; 48-bit LBA READ/WRITE DMA EXT; presented
//! behind the shared [`BlockDevice`] API.
...
fn alloc_ident(bytes: usize) -> u64 {
    ...
    assert!(!p.is_null(), "ahci: DMA alloc failed");
    p as u64
}
...
/// A polled AHCI port with a single command slot.
pub struct Ahci {
    port: u64,     // port register base
    clb: u64,      // command list (VA)
    ctba: u64,     // command table (VA)
    data_buf: u64, // 64 KiB bounce buffer (VA)
    capacity: u64,
}
...
impl Ahci {
    pub fn probe_nth(n: usize) -> Option<Ahci> {
        ...
        unsafe { Self::bringup(abar) }
    }

    unsafe fn bringup(abar: u64) -> Option<Ahci> {
        unsafe {
            ...
            let clb = alloc_ident(1024);
            let fb = alloc_ident(256);
            let ctba = alloc_ident(4096);
            w32(port + P_CLB, dma(clb) as u32);
            w32(port + P_CLB + 4, (dma(clb) >> 32) as u32);
            w32(port + P_FB, dma(fb) as u32);
            w32(port + P_FB + 4, (dma(fb) >> 32) as u32);
            // Slot 0 header -> the command table.
            write_volatile((clb + 8) as *mut u32, dma(ctba) as u32); // CTBA
            write_volatile((clb + 12) as *mut u32, (dma(ctba) >> 32) as u32);
            ...
            let mut a = Ahci { port, clb, ctba, data_buf: alloc_ident(DATA_MAX), capacity: 0 };
    ...
    unsafe fn issue(&mut self, cmd: u8, lba: u64, count: u16, bytes: u32, write: bool) -> bool {
        unsafe {
            ...
            write_volatile(self.clb as *mut u32, flags);
            write_volatile((self.clb + 4) as *mut u32, 0); // PRDBC
