// AAC Huffman codebooks — spectral and scalefactor tables from Symphonia
// (NihAV origin). MPL-2.0; see THIRDPARTY-LICENSES.md.
//
// Symphonia
...
//! Huffman tables + dequant lookup builders for AAC-LC.
...
/// Fast Huffman book: full LUT of size 2^max_len (max_len capped at 16 for
/// spectrum; SCF uses a 16-bit first stage + residual linear match).
pub struct HuffBook {
    /// Packed: high 8 bits = code length, low 24 bits = symbol index.
    /// 0 = invalid.
    lut: Vec<u32>,
    bits: u32,
    /// For codes longer than `bits`, residual (code, len, value).
    rest: Vec<(u32, u8, u16)>,
}

impl HuffBook {
    pub fn build(codes: &[u32], lens: &[u8]) -> Self {
        assert_eq!(codes.len(), lens.len());
        let max_len = lens.iter().copied().max().unwrap_or(0) as u32;
        // Cap direct LUT at 16 bits for memory; residual for longer (rare).
        let bits = core::cmp::min(max_len, 16);
        let size = 1usize << bits;
        let mut lut = alloc::vec![0u32; size];
        ...
        for (i, (&code, &len)) in codes.iter().zip(lens.iter()).enumerate() {
            ...
            if len <= bits {
                ...
                let n = 1usize << shift;
                let pack = (len << 24) | (val as u32);
                for k in 0..n {
                    // Prefer shorter codes (prefix-free books should not collide;
                    // still guard if table order puts a longer entry first).
                    let cur = lut[base + k];
                    if cur == 0 || (cur >> 24) > len {
                        lut[base + k] = pack;
                    }
    ...
    #[inline]
    pub fn decode(&self, br: &mut BitReader<'_>) -> Result<usize, &'static str> {
        let avail = br.bits_left() as u32;
        if avail == 0 {
            return Err("aac: huffman underrun");
        }
        let n = core::cmp::min(self.bits, avail);
        let peek = br.peek(n)?;
        // Align partial peek into the table: code is left-justified in `bits`.
        let idx = (peek as usize) << (self.bits - n) as usize;
        let e = self.lut[idx];
        if e != 0 {
            let len = e >> 24;
            if len <= n {
                let val = (e & 0x00ff_ffff) as usize;
                br.skip(len)?;
                return Ok(val);
            }
            // Table entry needs more bits than available.
            return Err("aac: huffman underrun");
        }
        // No LUT hit — maybe a residual code longer than `bits`.
        if !self.rest.is_empty() {
            let mut code = if n == self.bits {
                peek
            } else {
                // Rebuild from stream progressively for residual match.
                0
            };
            let mut len = if n == self.bits { self.bits } else { 0 };
            if n != self.bits {
                // Progressive from zero for short-stream residual (rare).
                loop {
                    if br.bits_left() == 0 {
                        break;
                    }
                    len += 1;
                    code = br.peek(len)?;
                    for &(c, l, v) in &self.rest {
                        if l as u32 == len && c == code {
                            br.skip(len)?;
                            return Ok(v as usize);
                        }
                    }
                    // Also try matching via progressive full table linear:
                    // fall through at max.
                    if len >= 24 {
                        break;
                    }
                }
                return Err("aac: huffman residual miss");
            }
            loop {
                for &(c, l, v) in &self.rest {
                    if l as u32 == len && c == code {
                ...
                if len >= 24 || br.bits_left() as u32 <= len {
                    break;
                }
                len += 1;
                code = br.peek(len)?;
            }
            return Err("aac: huffman long miss");
        }
        Err("aac: huffman invalid")
    }
}

#[inline]
pub fn quads_vals(idx: usize) -> (u8, u8, u8, u8) {
    AAC_QUADS[idx]
}

// --- math helpers for one-time table build ---------------------------------

fn libm_lnf(x: f32) -> f32 {
    if x <= 0.0 {
    ...
    let mut s = t;
    let mut p = t;
    for d in [3.0f32, 5.0, 7.0, 9.0, 11.0, 13.0, 15.0, 17.0] {
        p *= t2;
        s += p / d;
    }
    2.0 * s + e as f32 * core::f32::consts::LN_2
}
...
fn libm_expf(x: f32) -> f32 {
    ...
    let xn = x * log2e;
    let n = if xn >= 0.0 { xn as i32 } else { xn as i32 - 1 };
    let f = xn - n as f32;
    let y = f * core::f32::consts::LN_2;
    let mut s = 1.0f32;
    let mut t = 1.0f32;
    for d in 1..14 {
        t *= y / d as f32;
...
pub fn powf_f32(base: f32, exp: f32) -> f32 {
    ...
    libm_expf(exp * libm_lnf(base))
}

#[inline]
fn iquant(val: usize) -> f32 {
    powf_f32(val as f32, 4.0 / 3.0)
}

fn signed_pair(mod_: usize, cw: usize) -> (f32, f32) {
    let modulo_2 = mod_ >> 1;
    let a = cw / mod_;
    let b = cw % mod_;
...
fn unsigned_pair(mod_: usize, cw: usize) -> (f32, f32) {
    (iquant(cw / mod_), iquant(cw % mod_))
}

/// Precomputed dequant / scalefactor / Huffman tables, built once per decoder.
pub struct DequantTables {
    pub spectrum: [HuffBook; 11],
    pub scf: HuffBook,
    pub pair_vals: [Vec<(f32, f32)>; 6],
    pub esc_vals: Vec<(u16, u16)>,
    pub pow43: Vec<f32>,
    pub normal_scf: Vec<f32>,
    pub intensity_scf: Vec<f32>,
}

impl DequantTables {
    pub fn new() -> Self {
        let spectrum = [
            HuffBook::build(SPECTRUM_TABLES[0].codes, SPECTRUM_TABLES[0].lens),
            HuffBook::build(SPECTRUM_TABLES[1].codes, SPECTRUM_TABLES[1].lens),
            HuffBook::build(SPECTRUM_TABLES[2].codes, SPECTRUM_TABLES[2].lens),
            HuffBook::build(SPECTRUM_TABLES[3].codes, SPECTRUM_TABLES[3].lens),
            HuffBook::build(SPECTRUM_TABLES[4].codes, SPECTRUM_TABLES[4].lens),
            HuffBook::build(SPECTRUM_TABLES[5].codes, SPECTRUM_TABLES[5].lens),
            HuffBook::build(SPECTRUM_TABLES[6].codes, SPECTRUM_TABLES[6].lens),
            HuffBook::build(SPECTRUM_TABLES[7].codes, SPECTRUM_TABLES[7].lens),
            HuffBook::build(SPECTRUM_TABLES[8].codes, SPECTRUM_TABLES[8].lens),
            HuffBook::build(SPECTRUM_TABLES[9].codes, SPECTRUM_TABLES[9].lens),
            HuffBook::build(SPECTRUM_TABLES[10].codes, SPECTRUM_TABLES[10].lens),
        ];
        // SCF has max_len 19 — build with 16-bit LUT + residual.
        let scf = HuffBook::build(&SCF_CODEBOOK_CODES, &SCF_CODEBOOK_LENS);

        let pair_vals = [
            (0..SPECTRUM_TABLES[4].codes.len()).map(|cw| signed_pair(9, cw)).collect(),
            (0..SPECTRUM_TABLES[5].codes.len()).map(|cw| signed_pair(9, cw)).collect(),
            (0..SPECTRUM_TABLES[6].codes.len()).map(|cw| unsigned_pair(8, cw)).collect(),
            (0..SPECTRUM_TABLES[7].codes.len()).map(|cw| unsigned_pair(8, cw)).collect(),
            (0..SPECTRUM_TABLES[8].codes.len()).map(|cw| unsigned_pair(13, cw)).collect(),
            (0..SPECTRUM_TABLES[9].codes.len()).map(|cw| unsigned_pair(13, cw)).collect(),
        ];
        let esc_vals: Vec<(u16, u16)> = (0..SPECTRUM_TABLES[10].codes.len())
            .map(|cw| ((cw / 17) as u16, (cw % 17) as u16))
            .collect();
        let mut pow43 = alloc::vec![0f32; 8192];
        for i in 0..8192 {
            pow43[i] = iquant(i);
        }
        let mut normal_scf = alloc::vec![0f32; 256];
        for i in 0..256 {
            normal_scf[i] = powf_f32(2.0, 0.25 * (i as f32 - 156.0));
        }
        let mut intensity_scf = alloc::vec![0f32; 256];
        for i in 0..256 {
            intensity_scf[i] = powf_f32(0.5, 0.25 * (i as f32 - 155.0));
        }
        Self {
            spectrum,
            scf,
