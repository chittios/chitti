//! IMDCT + window + overlap-add for AAC filterbank synthesis.
//!
//! From Symphonia / NihAV (MPL-2.0). See THIRDPARTY-LICENSES.md.

use super::imdct::Imdct;
use super::tables::*;
...
impl Dsp {
    pub fn new() -> Self {
        let mut kbd_long_win = [0.0f32; 1024];
        let mut kbd_short_win = [0.0f32; 128];
        ...
        let mut sine_long_win = [0.0f32; 1024];
        let mut sine_short_win = [0.0f32; 128];
    ...
    pub fn synth(
        ...
        match seq {
            ...
            _ => {
                // Unknown sequence: silence output, keep delay
                dst[..1024].fill(0.0);
                return;
            }
        }
