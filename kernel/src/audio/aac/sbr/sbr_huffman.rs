...
///
/// Returns [`"aac/sbr: huffman invalid"`] if no codeword of length up to
...
pub fn sbr_huff_dec(
    ...
    let mut len: u32 = 0;
    loop {
        codeword = (codeword << 1) | reader.read_bits(1).map_err(|_| "aac/sbr: huffman invalid")?;
        len += 1;
        ...
        if len >= SBR_HUFF_MAX_CODE_LEN {
            return Err("aac/sbr: huffman invalid");
        }
    }
}
