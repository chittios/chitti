...
//!
//! Same shape as [`super::sbr_huffman`]: each table is `[(u8, u32); N]`
...
/// entry `(length, codeword)`. Returns [`"aac/sbr: ps data invalid"`] if no
/// codeword of length up to [`PS_HUFF_MAX_CODE_LEN`] matches (a
/// corrupt or truncated `ps_data()` payload).
pub fn ps_huff_dec(
    reader: &mut crate::audio::aac::bits::BitReader<'_>,
    table: &[(u8, u32)],
    lav: i32,
) -> Result<i32> {
    ...
    let mut len: u32 = 0;
    loop {
        codeword = (codeword << 1) | reader.read_bits(1).map_err(|_| "aac/sbr: ps data invalid")?;
        len += 1;
