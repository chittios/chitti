...
agent breaks all three. Its instruction stream is generated stochastically, its
request set is unbounded, and its apparent intent derives from data it has read
--- so whoever controls that data controls the requests. Practice answers above
...
The implementation is a dual-architecture Rust kernel in which the model runs
in-kernel with no I/O of its own, so there is no way around the ABI; the
mechanism is 2.4k lines. Measured: the authorization decision costs
1.4\,$\mu$s ($3\times10^{-5}$ of one decoded token), and an
injection corpus is refused 13 times out of 13 --- while removing provenance
lets 10 through and removing attenuation all 13. The cost is a quarter of the
