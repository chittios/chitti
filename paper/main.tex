...
binding type, so a new binding does not compile until someone says what it does. Relatedly, a census of which
...
\textbf{Result: it loses on both axes} (Table~\ref{tab:e4}). It permits
\textbf{4 of 13} attacks the strict rule refuses, and it recovers \textbf{none}
of the false refusals it exists to remove. The one benign refusal it removed was
the warranted exfiltration case.
...
12}; removing attenuation as well takes it to \textbf{13 of 13}. So the two
...
computed after the fact can see that hop. Real per-value provenance therefore
means tagging what the model emits by what it read: a substantially larger
mechanism than the lattice it would replace, sitting at the boundary rather than
below it, and one whose own soundness argument we do not yet have.
...
The confirmation now names the \emph{provenance}: when the turn is tainted the
dialogue shows the ingested text that is the reason for asking, so the human is
deciding about a source rather than about an operation. That is the decision the
policy actually needs from them, and the only one they have information the
kernel lacks about. What it cannot yet show is \emph{which} source --- the
session tags each message with a provenance but not an origin, so the dialogue
quotes the untrusted text rather than naming \code{evil.example}. Threading an
origin through every ingestion path is the remaining work, and it would make the
prompt answerable at a glance rather than by reading.
...
What it still is not is attested. A compromised kernel can recompute the whole
chain, so this defends a reader against a doctored snapshot, not against a
dishonest machine; and the log remains in memory, so it does not survive a
reboot. Both need a key and a place to put it that this system does not yet
have.
