...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/redteam.png}
\caption{E2 and E4 as they actually run, screenshotted from the guest's own
framebuffer (the per-attack detail is Table~\ref{tab:e2}; the full-width table
view is \code{figures/redteam\_table.png}). The left pane is the comparison this
evaluation exists to make, printed by the machine under test: zero attacks
permitted under the full policy, 9 of 12 with provenance removed, 12 of 12 under
ambient authority, and the effective counts beside each. The right pane is not
decoration --- it is the \code{browser-navigate-egress} attack \emph{executing}
under the ambient baseline, stuck on ``Loading'' because the corpus points every
egress at the loopback discard port. A permitted attack therefore demonstrably
runs, and demonstrably cannot reach the network.}
\label{fig:redteam}
\end{figure}
