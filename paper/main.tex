...
\emph{program}: untrusted, but deterministic, with knowable effects. An
LLM agent breaks all three properties. Its instruction stream is generated
stochastically, its request set is unbounded, and its apparent intent is derived
from data it has read --- so whoever controls that data controls the requests.
Practice answers above the OS, with prompt guardrails and per-call approval, while the agent holds the user's full ambient authority.
...
executes capability-checked primitives. Model output never causes an effect
directly. A fixed registry generates a prefix-closed grammar, applied in the
model's own token space and revalidated at the executor; capabilities attenuate
on delegation and a scope gate binds each to concrete paths and hosts; and a
provenance lattice makes \emph{taint a syscall argument}, refusing destructive
primitives justified by untrusted content --- Biba integrity, low-water-marked
over the live context. Every attempt, denials included, is audited.

The implementation is a dual-architecture Rust kernel in which the
model runs in-kernel with no I/O of its own, so there is no way around the ABI;
the mechanism is 2.4k lines. Measured on the running system: the authorization
decision costs 1.4\,$\mu$s ($3\times10^{-5}$ of one decoded token), and an
injection corpus is refused 12 times out of 12 --- while removing provenance
lets 9 through and removing attenuation all 12. The cost is the headline
liability: half the \emph{legitimate} irreversible operations in a benign suite
are refused, because turn-granular taint cannot tell which data justified them.
\end{abstract}
