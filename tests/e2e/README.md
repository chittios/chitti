...
| *os group* | every shell command: `/help /info /datetime /disks /lspci /mounts /ls /skills /shortcuts /mode /think /agents /ui /ktrace /close /top /clear /wifi`, plus `memory` (add/get/list), `session` (save/clear/resume), `help_restart` (`/help` lists `/restart` + `/memory`) |
