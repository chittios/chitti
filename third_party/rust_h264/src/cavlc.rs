...
/// `spin::Once` does not provide `get_or_init`; thin wrapper for the CAVLC LUTs.
trait OnceExt<T> {
    fn get_or_init<F: FnOnce() -> T>(&self, f: F) -> &T;
}
impl<T> OnceExt<T> for Once<T> {
    fn get_or_init<F: FnOnce() -> T>(&self, f: F) -> &T {
        self.call_once(f);
        // SAFETY: call_once guarantees initialization.
        self.get().expect("Once initialized")
    }
}
...
static LUT_COEFF_NC0: Once<Vec<CoeffEntry>> = Once::new(); // 16 bits → 64 KiB
static LUT_COEFF_NC2: Once<Vec<CoeffEntry>> = Once::new(); // 14 bits → 16 KiB
static LUT_COEFF_NC4: Once<Vec<CoeffEntry>> = Once::new(); // 10 bits →  1 KiB
static LUT_COEFF_CHROMA_DC: Once<Vec<CoeffEntry>> = Once::new(); //  8 bits → 256 B

#[allow(clippy::declare_interior_mutable_const)]
static LUT_TOTAL_ZEROS: [Once<Vec<U8Entry>>; 15] = [Once::new(), Once::new(), Once::new(), Once::new(), Once::new(), Once::new(), Once::new(), Once::new(), Once::new(), Once::new(), Once::new(), Once::new(), Once::new(), Once::new(), Once::new()];
static LUT_TOTAL_ZEROS_CHROMA: [Once<Vec<U8Entry>>; 3] = [Once::new(), Once::new(), Once::new()];
static LUT_RUN_BEFORE: [Once<Vec<U8Entry>>; 7] = [Once::new(), Once::new(), Once::new(), Once::new(), Once::new(), Once::new(), Once::new()];
