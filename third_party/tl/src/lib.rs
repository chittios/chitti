// VENDOR DIVERGENCE: `no_std` + `alloc`. Upstream is `std`-only, but only
// shallowly -- see DIVERGENCES.md. The `#![doc = include_str!("../README.md")]`
// line is dropped because the README is not vendored.
//! `tl` — a fast, non-spec HTML parser (vendored from crates.io 0.7.8, MIT).
//!
//! Vendored into ChittiOS as an **alternative** to `browser::html`, selectable
//! at runtime with `/html ours|tl`. See DIVERGENCES.md for the `no_std`
//! conversion.
#![no_std]
