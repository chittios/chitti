...
impl<'a> Parser<'a> {
    ...
    fn read_ident(&mut self) -> Option<&'a [u8]> {
        ...
        // VENDOR DIVERGENCE (ChittiOS): `util::is_ident` counts `/` as an
        // identifier byte, so `<br/>` reads its name as `br/`. That name is not
        // in `VOID_TAGS` and the following `expect_and_skip_cond(b'/')` finds
        // `>` instead of `/`, so the tag is treated as an ordinary open tag and
        // **pushed onto the stack** -- every later element nests inside it. A
        // page of `<br/><hr/><div>` comes out as `br/ > hr/ > div` instead of
        // three siblings. The same applies to attribute names (`<input
        // disabled/>` reads `disabled/`).
        //
        // A trailing `/` is never part of a tag or attribute name in HTML, so
        // it is trimmed and the stream rewound one byte -- the caller's
        // self-closing check then sees the `/` it was always looking for. Fixed
        // here rather than in `is_ident` because `/` is load-bearing elsewhere
        // in that predicate (the query-selector tokenizer shares it).
        //
        // Found by `browser::html_tl`'s cross-engine tree-equality test, on
        // markup our own `html::preprocess` emits.
        if end > 0 && self.stream.data()[start + end - 1] == b'/' {
            self.stream.idx -= 1;
            return Some(self.stream.slice(start, start + end - 1));
        }
