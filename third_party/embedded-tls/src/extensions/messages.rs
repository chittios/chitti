...
extension_group! {
    pub enum EncryptedExtensionsExtension<'a> {
        ...
        MaxFragmentLength(MaxFragmentLength),
        // Informational and unused by this client. Parse it as opaque bytes so a
        // server that advertises groups this crate's `NamedGroup` enum doesn't
        // know (e.g. post-quantum hybrids like X25519MLKEM768) — or more than
        // the old fixed cap of 10 — doesn't abort the whole handshake. This is
        // what broke `/http` to upload.wikimedia.org and other modern CDNs.
        // (Chitti in-tree patch.)
        SupportedGroups(Unimplemented<'a>),
        UseSrtp(Unimplemented<'a>),
        Heartbeat(Unimplemented<'a>),
        ApplicationLayerProtocolNegotiation(Unimplemented<'a>),
        ClientCertificateType(Unimplemented<'a>),
