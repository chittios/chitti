...
// ChittiOS patch: vendored + no_std'd so the just-engine JS parser can run in
// the bare-metal kernel (see THIRDPARTY-LICENSES.md). `alloc` provides
// Vec/String/Box/Rc + the vec!/format! macros crate-wide.
#![no_std]
