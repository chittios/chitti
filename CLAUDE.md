...
> **STANDING RULE — the kernel is dual-architecture (x86_64 + aarch64), and functionality must not diverge between them.** Every change must build and work for BOTH arches. Never guard behaviour behind `target_arch` unless it is genuinely arch-specific (a driver, an instruction) — and then provide the equivalent for the other arch behind the same API, never a stub that drops a feature. After any change, verify both: `cargo xtask build -arch x86_64` + `cargo xtask test` (103/103) **and** `cargo xtask build -arch aarch64` (and boot it via `-arch aarch64` when the change is boot-visible). If a capability exists on one arch, it exists on the other.
>
> **Parity status (audited 2026-07):** every user-visible capability exists on both arches — chat/inference, storage (`/disks /ls /mkfs /mkext4 /ext4read`), `/install`, synapse ext4 persistence, UEFI boot (x86: Limine ISO/OVMF; aarch64: `stub/` UEFI bootloader + AAVMF), framebuffer (GOP | ramfb), keyboard (PS/2+xHCI | virtio-input), SMP, `/exit`. The only `target_arch` gates are per-arch drivers behind shared facades (`block::DiskDevice`/`probe_disk_nth`, tensor kernels, serial, mm, context switch) and boot machinery. **A new feature must ship on both arches in the same change.**
